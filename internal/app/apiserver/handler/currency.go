...
// getFactorByCurrency определяет курс рубля относительно валюты currency
// возвращает либо число, на которое следует умножить сумму в рублях и nil
// либо 0 и ошибку, если такой валюты нет в списке возможных
// для получения курса валют используется сторонее API - https://exchangeratesapi.io/
func getFactorByCurrency(currency string) (float64, error) {

	// GET запрос для получения курса
	url := fmt.Sprintf("http://api.exchangeratesapi.io/v1/latest?access_key=%s", os.Getenv("API_KEY"))
	resp, err := http.Get(url)
	if err != nil {
		return 0, fmt.Errorf("error with GET request to %s: %s", url, err.Error())
	}

	// Проверяем код ответа
	if resp.StatusCode > 299 {
		return 0, fmt.Errorf("response failed with status code: %d and\nbody: %s\n", resp.StatusCode, resp.Body)
	}
	...
	if err != nil {
		return 0, fmt.Errorf("error with decoding request body: %s", err.Error())
	}

	// Закроем тело ответа
	resp.Body.Close()

	// Так как бесплатная версия API как базовую величину выдает евро
	// Сначала надо перевести рубли в евро, а затем в currency
	factor1, err := getField(rates, "RUB")
	if err != nil {
		return 0, err
	}
	factor2, err := getField(rates, currency)
	if err != nil {
		return 0, err
	}
...
// AutoGenerated - структура для представление данных с API для курса валют
